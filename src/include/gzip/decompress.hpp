...
public:
  ...
  void decompress(OutputType &output, const char *data,
    ...
    // better compression at the expense of memory usage. This range of values
    // also changes the decoding type:
...
#pragma GCC diagnostic pop
    ...
    do {
      ...
      if (resize_to > max_) {
        inflateEnd(&inflate_s);
        throw std::runtime_error("size of output string will use more memory "
                                 "then intended when decompressing");
      }
      output.resize(resize_to);
      inflate_s.avail_out = static_cast<unsigned int>(2 * size);
      inflate_s.next_out =
          reinterpret_cast<Bytef *>(&output[0] + size_uncompressed);
